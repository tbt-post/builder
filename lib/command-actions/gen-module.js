    ...
    glob(path.join('./', localRootPath, folderName, '/**/!(common).spec.json'), {}, (err, files) => {
        ...
        if (files.length) {
            ...
            files.forEach(file => {
                ...
                data.url = `./${path.relative(basePath, data.filePathJs)}`;
                data.loader = `{{func}}()=>{return import(/* webpackChunkName: '${data.folderName.replace(/\//gi, '.')}.${data.fileName}' */ './${path.relative(basePath, data.filePathJs)}')}{{func}}`;
                result[data.id] = data
            });
